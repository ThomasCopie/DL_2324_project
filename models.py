...
class Model2(nn.Module):
    """takes num_entries 1D signals of length len_signal and outputs a vector of length num_classes. Lighter model"""
    ...
    def __init__(self, num_entries = 12, len_signal = 4980, num_classes = 5):
        ...
        self.convs = nn.ModuleList([nn.Sequential(
            ...
            nn.Conv1d(64, 64, kernel_size = 7, padding = 3),
            nn.MaxPool1d(4),
            nn.BatchNorm1d(64)) for _ in range(num_entries)])
        
        self.output_size = len_signal // 2 // 4 // 4 // 4 // 4 // 4
        self.fc1 = nn.ModuleList([nn.Linear(64 * self.output_size, num_classes) for _ in range(num_entries)])

        self.avg_weights = nn.Parameter(torch.ones(num_entries) / num_entries)
    ...
    def forward(self, x):
        ...
        weights = torch.sigmoid(self.avg_weights)
        weights = weights / weights.sum()
        outputs = (outputs * weights.unsqueeze(0).unsqueeze(2)).sum(dim=1)
        return outputs
